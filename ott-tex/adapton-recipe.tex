...
\usepackage{charter}
\usepackage{xcolor}
\usepackage[alternateNonterms,lineBreakHack]{ottalt}
...
%\usepackage{fontspec}
%\setmonofont{Fira Code} % or JetBrains Mono, Source Code Pro, etc.

% \usetikzlibrary{positioning,calc,3d,backgrounds}

\usepackage[colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue]{hyperref}
\usepackage{cleveref}
% \usepackage{refcheck} % -- refcheck is not compatible with cleveref, after all.

\usepackage{nag}

\lstdefinelanguage{Fumola}{
  morekeywords={switch,case,public,func,thunk,if,else,let,force,do,type,module},
  sensitive=true,
  literate=
    {:=}{{{\bfseries\color{blue}:=\ }}}2
    {@}{{{\bfseries\color{blue}@}}}1
}
\lstdefinestyle{Fumola}{
  language=Fumola,
...
\newcommand{\SeqTreeBinaryNode}[2]{\begin{tabular}{|c|}\hline{\ttfamily\itshape \color{gray}{#1}}\\\hline {\tiny #2}\\\hline\end{tabular}}
\newcommand{\SeqTreeElement}[2]{\begin{tabular}{|c|}\hline{\ttfamily\itshape\color{gray}{#1}}\\\hline\bfseries #2\\\hline\end{tabular}}
...
\item \Cref{sec:symbolic-space-and-time} introduces the idea of
...
\item \Cref{sec:programs} gives some prototypical syntax
...
\item \Cref{sec:reference-dynamics} presents the reference dynamics.

\item \Cref{sec:graphical-dynamics} presents the graphical dynamics.

\end{itemize}

The %first 
appendix gives draft sections that are in progress.

\begin{itemize}

\item \Cref{sec:collecting-semantics} presents a collecting semantics for the reference semantics.

\item \Cref{sec:abstract-semantics} presents a decidable, finite abstraction of the collecting semantics.

\item \Cref{sec:from-scratch-consistency} presents a meta theory
...
\item \Cref{sec:collections} introduces the recipe's approach to
...
\item There is a pending implementation.  Once more
  complete, we will include a description of it, and
...
%
% The second appendix gives complete supplementary material.
%
% to do
%
...
%
\Cref{sec:refsem-navigation} describes the interaction of
...
\subsection{Symbolic space, time and pointers.}
\label{sec:symbolic-space}
\label{sec:symbolic-time}
...
%
\Cref{sec:refsem-navigation,sec:refsem-undelay}
...
In this section, we define a \emph{reference semantics} for the \emph{store behavior} of the Adapton Recipe.
%
In contrast with the graphical semantics (\Cref{sec:graphical-dynamics}),
this view deliberately omits the demanded computation graph (DCG).
%
Instead, it defines how a program uses first-class symbols to
globally name space and time for stored data and thunks.

As we argue in later sections,
to reason about the store behavior of programs using the Adapton Recipe,
it suffices to reason about this semantics alone (formally stated in \Cref{sec:from-scratch-consistency}).
%
We leverage this fact to define a collecting semantics and abstract semantics 
based on this reference semantics, and notably,
independently 
of the graphical semantics (\Cref{sec:collecting-semantics,sec:abstract-semantics}).

The reference semantics can be implemented by an interpreter with five inputs, and two outputs
(for now, ignoring dynamic errors that arise from violating the abstract semantics):
...
\item \textbf{Inputs:}
  ...
  and expression~$[[e]]$
\item \textbf{Outputs:}
  store~$[[Store2]]$ and value~$[[v]]$
\end{itemize}

We define paths as a list of thunk pointers:

\ottgrammartabular{
  \ottPath
}

We define the reference semantics as a seven place relation as follows:
...
The relation uses nineteen rules, which we discuss in groups below.
%
The entire set of rules is given again in \Cref{fig:reference-semantics}.
...
as \rref{E-forceThunk} handles the ordinary, non-pointer case.
...
%
To fill an empty cell, \rref{E-emptyForce} evaluates the thunk body (under
...
\input{sec-collecting-semantics-out}

\input{sec-abstract-semantics-out}
...
each each collection type, such as set, partial map, or sequence.
%
...
\newcommand{\CaptionToggle}[1]{} % off

\begin{figure}
  \begin{tabular}[t]{|p{0.382\textwidth}|p{0.618\textwidth}|}
  \hline
  \begin{minipage}[t]{0.382\textwidth}
  \vspace{0pt} % <-- Required for top alignment inside minipage
  \lstinputlisting[language=Fumola, style=Fumola,\CaptionToggle{\texttt{SeqTree} type.}, captionpos=b, label={lst:SeqTree}]{../examples/SeqTree.SeqTree.fumola-type-listing}
  \lstinputlisting[language=Fumola, style=Fumola]{../examples/SeqTree.Element.fumola-type-listing}
  \lstinputlisting[language=Fumola, style=Fumola]{../examples/SeqTree.Binary.fumola-type-listing}
  \lstinputlisting[language=Fumola, style=Fumola,\CaptionToggle{\texttt{Element}, \texttt{Binary} and \texttt{SeqTree\_} types.}, captionpos=b, label={lst:SeqTree_}]{../examples/SeqTree.SeqTree_.fumola-type-listing}
  \end{minipage}
  &
  \begin{minipage}[t]{0.618\textwidth}
  \lstinputlisting[language=Fumola, style=Fumola,\CaptionToggle{\texttt{reduceUp}, a function over \texttt{SeqTree}.}, captionpos=b, label={lst:SeqTree-reduceUp}]{../examples/SeqTree.reduceUp.fumola-func-listing}
  \end{minipage}
  \\
  \hline
\end{tabular}
...
\begin{figure}
  \begin{center}
  \input{mergeSort-inputSeq}
  % todo -- input an image of the original sequence, as a linked list
  \end{center}
  \caption{\texttt{mergeSort} input sequence, as a harmonious tree.}
\end{figure}
...
\begin{tikzpicture}[
    thunkCell/.style={rectangle, rounded corners=5pt, draw, thick, fill=white!90!gray, inner sep=5pt},
    nonthunkCell/.style={rectangle, rounded corners=0.6pt, draw, fill=white!90!gray, inner sep=5pt},
    cons/.style={rectangle, rounded corners=1.3pt, draw, fill=white},
    description/.style={draw=none, fill=white},
    pointer/.style={->, thick, >=Stealth, dotted},
    connection/.style={->, thick, color=blue!50!black, opacity=0.7},
    edge/.style={->, thick, >=Stealth, blue!50!black},
    edgeNum/.style={near start, above, draw=none},
    fieldLabel/.style={near start, above, draw=none, fill=white},
    cellName/.style={draw=none, fill=white},
  ]
  \begin{scope}
    \node[thunkCell] (A1) at (0,0) {$A$};
    \node[description] (d1) at (5.5,0) {Thunk cell at space~$A$};

    \node[nonthunkCell] (A2) at (0,-1) {$A$};
    \node[description] (d2) at (5.5,-1) {non-thunk cell at space~$A$};

    \node[cons] (cons1) at (0,-2) {$s~|~x~|~p$};
    \node[description] (d3) at (5.5,-2) {Data record, tuple~$(s,x,p)$};

    \node[nonthunkCell,minimum width=52pt,minimum height=29pt] (A3) at (0,-3.5) {};
    \node[cons] (cons2) at (0,-3.5) {$s~|~x~|~p$};
    \node[description] (d4) at (5.5,-3.5) {Cell~$A$ holds tuple~$(s,x,p)$};
    \node[cellName, left=1pt of A3] (AThreeName) {$A$};
