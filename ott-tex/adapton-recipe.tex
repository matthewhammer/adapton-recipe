...
\begin{definition}[Tree representation]
  Given a set of mathematical structures~$\mathcal{S}$ with a certain type~$\tau$ (like
  sequence, set or finite map of some other element type) we say
  that $\ottkw{tree}$ is a \emph{(binary) tree representation} if:
  \begin{itemize}
  \item $\ottkw{tree}$ has a representation that is isomorphic to the
    following recursive sum type:
    \[
    \texttt{Tree<X,Y>} \equiv \texttt{\{\#empty; \#elm(X); \#bin(Tree, Y, Tree)\}}
    \]
  \item Where type $X$ carries the data associated with the elements of each $s \in S$.
    
  \item And where, by contrast, $Y$
    carries any \emph{meta data} needed to \emph{locate} the elements of type $X$,
    but \emph{not} those elements themselves.

  \item In other words, $\ottkw{tree}$ has a representation that can
    be traversed like a binary tree, with element data at the leaves
    and navigation data at the binary nodes; and, this representation
    can be generated with those same constructor forms.
...
A binary tree representation may not be adequate or practical if it
lacks certain properties related to how it adapts to change, and
integrates with other data structures around it.
%
