...
each of these collection types.

Each such binary tree requires two properties, which the recipe satisfies:

\begin{itemize}  
\item \textbf{change stable}: similar mathematical structures
  correspond to similar binary trees, with many identical subtree
  structures.

\item \textbf{balanced}: the size of the left and right subtrees of
  every tree are \emph{asymptotically equal}.  For instance, for every
  binary tree, the left and right subtree sizes are within a factor of
  $[1/b, b]$ of each other, for some constant $b$ (e.g., consider $b$
  as $2$, or possibly even larger values).
\end{itemize}

The rest of the recipe's approach to collection-based algorithms
relies on these two properties for each binary tree, regardless of
