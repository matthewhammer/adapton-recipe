...
\begin{tikzpicture}[scale=2.0, every node/.style={draw, scale=0.5},
  layerA/.style={yslant=-0.5, xslant=0.5, xshift=-2cm},
...
% Layer B: Tree B
  \begin{scope}[layerB, every node/.style={circle, draw, fill=white!80!gray}]
  \node (B1) at (0,0) {B1};
  \node (B2) at (-1,-1.5) {B2};
  \node (B3) at (1,-1.5) {B3};
  \node (B4) at (-1.5,-3) {B4};
...
\draw[connection] (B1) .. controls +(up:1cm) and +(down:1cm) .. (A1);
\draw[connection] (B2) .. controls +(up:1cm) and +(down:1cm) .. (A2);
\draw[connection] (B3) .. controls +(up:1cm) and +(down:1cm) .. (A3);

\draw[connection] (B4) .. controls +(up:1cm) and +(down:1cm) .. (A4);
\draw[connection] (B5) .. controls +(up:1cm) and +(down:1cm) .. (A5);
\draw[connection] (B6) .. controls +(up:1cm) and +(down:1cm) .. (A6);
\draw[connection] (B7) .. controls +(up:1cm) and +(down:1cm) .. (A7);
