...
demanded.
%
Depending on the status of its local trace edges, there are several
possible situations, each captured by one rule below.
...
First, the local edges of a node may all be cleanable, either because
they are already clean, or because one or more were previously marked
dirty but are now consistent with their dependencies, and thus may now
be marked clean.
%
This case of a cleanable node trace~$[[t]]$ (and the subcases just
mentioned) are captured by the judgement~$[[Graph1; Path; Moment; m;
    Space |- clean(t) !! Graph2]]$.

% xxx

Second, at least one edge may not be cleanable because it witnesses an
effect with an outdated value.
%
The judgement~$[[Graph1; Path; Moment; m; Space |- notCleanable(n) !! Graph2]]$
captures this situation for an edge named~$[[n]]$, and it assumes that
all the dependencies for $[[n]]$ have already been cleaned, somehow.
...
When dirty~(rule CTXXdirtyIntoClean), the rule recurs into cleaning
the target node, and then insists that the edge-action and target-node
values match (see premise $[[value(A) = value(Graph2, qq)]]$).
%
It relies on this fact to justify updating the edge status to be
clean.

This cleaning judgement is not derivable for any edge when this pair of values
do not match, after visiting the edge target.
%
Instead, the cleaning traversal derives $[[Graph1; Path; Moment; m;
    Space |- misaligned(n) !! Graph2]]$ for this ``misaligned'' edge,
and reevalutes the node to replace its local trace with updated edges,
and updated actions and action-values, wherein the value pairs across each edge are each matching.
