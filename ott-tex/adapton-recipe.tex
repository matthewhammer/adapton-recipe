...
%
The filled cache state records an the a store snapshot~$[[Store]]$
just before that thunk last ran, and the result value~$[[v]]$ of that
run.

The notion of cache reuse within the reference semantics requires that the current
store be an ``aligned extension'' of the store snapshot from when the
thunk last ran.
%
This aligned-store extension relationship permits no
mutable effects other than thunk evaluation, making it considerably
more limited than the graphical semantics, which uses
the graphical structure of the DCG to identify fine-grained
opportunities for reuse, and if necessary, repair.

The structure of the graph is motivated by the need to avoid the
various inpracticalities of the reference semantics, but retain its input-output behavior.
%
...
graphical semantics, and vice versa; the difference is that the
derivations in the graphical semantics will always be similar to or smaller than
those of the reference semantics in size, and sometimes \emph{much} smaller.
%
